# bronze_ingest_nri.py
"""
Glue Job: bronze_ingest_nri
...
def main() -> None:
    ...
    from ingestion.nri import ingest_nri
    ...
    manifest = ingest_nri(bucket=bucket, bronze_prefix=bronze_prefix, run_dt=run_dt)

    manifest_key = ops_key(ops_prefix, "bronze_ingestion", "nri", run_dt, "manifest.json")
    put_json(bucket, manifest_key, manifest)

    print(f"[OK] NRI Bronze ingestion complete. Manifest: s3://{bucket}/{manifest_key}")
