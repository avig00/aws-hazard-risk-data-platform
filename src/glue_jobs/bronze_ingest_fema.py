# bronze_ingest_fema.py
"""
Glue Job: bronze_ingest_fema
...
def main() -> None:
    ...
    from ingestion.fema import ingest_fema
    ...
    manifest = ingest_fema(bucket=bucket, bronze_prefix=bronze_prefix, run_dt=run_dt)

    manifest_key = ops_key(ops_prefix, "bronze_ingestion", "fema", run_dt, "manifest.json")
    put_json(bucket, manifest_key, manifest)

    print(f"[OK] FEMA Bronze ingestion complete. Manifest: s3://{bucket}/{manifest_key}")
